-- Crear tabla para gestión de estructura (versión corregida)
...
-- Agregar la clave foránea para padre_id después de crear ambas tablas
ALTER TABLE `elementos_estructura` 
ADD CONSTRAINT `fk_elemento_padre` 
FOREIGN KEY (`padre_id`) REFERENCES `elementos_estructura`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- Insertar estructura por defecto (sin clave foránea a usuarios por ahora)
